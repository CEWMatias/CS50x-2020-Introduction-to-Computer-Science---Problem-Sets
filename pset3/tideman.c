...
int compare(const void *elem1, const void *elem2)
{
    ...
    int sencond_margin = preferences[s.winner][s.loser] - preferences[s.loser][s.winner];
    return sencond_margin - first_margin;
}
...
void lock_pairs(void)
{
    for (int i = 0; i < pair_count; i++)
    {
        ...
        //check for cycle in this directed gragh starting with the ith candidate
        if (has_cycle(pairs[i].winner)) // if cycle exists starting from i
        {
            locked[pairs[i].winner][pairs[i].loser] = false; // remove the pair
        }
